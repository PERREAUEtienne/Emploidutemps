...
public class FenetreProf extends JFrame implements ActionListener, ItemListener {
    ...
    public ArrayList<String> afficherVendredi() throws SQLException, IOException {
        ...
        try {
            // recupérér les résultats de la requete
                listeVendredi = maconnexion.remplirChampsRequete("SELECT seance.id,cours.nom , groupe.nom,salle.nom,site.nom,type_cours.nom, utilisateur.nom,promotion.nom "
                    + "FROM seance,seance_groupes,groupe,cours,salle,seance_salles,site,type_cours,utilisateur,promotion "
                    + "WHERE utilisateur.droit=3 "
                    + "AND seance.id_cours=cours.id "
                    + "AND seance.id=seance_salles.id_seance "
                    + "AND salle.id=seance_salles.id_salle "
                    + "AND site.id=salle.id_site "
                    + "AND seance.id_type=type_cours.id AND seance.id=seance_groupes.id_seance "
                    + "AND groupe.id=seance_groupes.id_groupe "
                    + "AND groupe.id_promotion=promotion.id "
                    + "ORDER BY seance.heure_debut)"  );
    ...
    public ArrayList<String> afficherLundi2(int i) throws SQLException {
        ArrayList<String> listeLundi = null;
        try {

            // recupérér les résultats de la requete selectionnee
              listeLundi = maconnexion.remplirChampsRequete("SELECT email"
                          + "FROM utilisateur"
    ...
    public void affichage2(int i) throws SQLException, IOException{
        try {
                    // tentative de connexion si les 4 attributs sont remplis
                    //maconnexion = new Connexion("jps", "root", "");
                    maconnexion = new Connexion("edp", "root", "");
                    ...
                    afficherVendredi();/*
                    afficherMardi(nom);
                    afficherMercredi(nom);
                    afficherJeudi(nom);
                    afficherVendredi(nom);*/
                } catch (ClassNotFoundException cnfe) {
                    System.out.println("Connexion echouee : probleme de classe");
                    cnfe.printStackTrace();
                }
